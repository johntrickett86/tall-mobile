...
@section('content')
    ...
    <div class="relative min-h-screen bg-light-50/50 dark:bg-gradient-to-b dark:from-light-950 dark:to-light-900 sm:flex sm:justify-center sm:items-center">

        <x-partials.headers.mobile-header-main :scroll-hide="true" />

        @if (Route::has('login'))
            <div class="p-6 text-right sm:fixed sm:top-0 sm:right-0 hidden lg:flex">
                @auth
                    <div class="flex justify-end items-center space-x-4">
                        <a href="{{ route('home') }}" class="font-semibold text-light-600 hover:text-light-900 dark:text-light-400 dark:hover:text-white focus:outline focus:outline-2 focus:rounded-sm focus:outline-primary-500">Home</a>
                        <form method="POST" action="{{ route('logout') }}" class="inline">
                            @csrf
                            <button type="submit" class="font-semibold text-light-600 hover:text-light-900 dark:text-light-400 dark:hover:text-white focus:outline focus:outline-2 focus:rounded-sm focus:outline-primary-500">Logout</button>
                        </form>
                    </div>
                @else
                    <div class="flex justify-end items-center space-x-4">
                        <a href="{{ route('login') }}" class="font-semibold text-light-600 hover:text-light-900 dark:text-light-400 dark:hover:text-white focus:outline focus:outline-2 focus:rounded-sm focus:outline-primary-500">Log in</a>

                        @if (Route::has('register'))
                            <a href="{{ route('register') }}" class="font-semibold text-light-600 hover:text-light-900 dark:text-light-400 dark:hover:text-white focus:outline focus:outline-2 focus:rounded-sm focus:outline-primary-500">Register</a>
                        @endif
                    </div>
                @endauth
            </div>
        ...
        <div class="p-6 mx-auto max-w-7xl lg:p-8">
            <div class="flex justify-center">
                <x-logo class="w-auto h-16 text-primary-600 dark:text-primary-500" />
            </div>
            <div class="mt-16">
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:gap-8">
                    <a href="https://tailwindcss.com/docs" target="_blank" class="scale-100 p-6 bg-white dark:bg-light-800/50 dark:bg-gradient-to-bl from-light-700/50 via-transparent dark:ring-1 dark:ring-inset dark:ring-white/5 rounded-lg shadow-2xl shadow-light-500/20 dark:shadow-none flex motion-safe:hover:scale-[1.01] transition-all duration-250 focus:outline focus:outline-2 focus:outline-primary-500">
                        <div>
                            <div class="flex items-center justify-center w-16 h-16 rounded-full bg-primary-50 dark:bg-primary-900/20">
                                <svg class="text-primary-400 fill-current w-7 h-7" viewBox="0 0 50 31" fill="none" xmlns="http://www.w3.org/2000/svg"><g clip-path="url(#a)"><path fill-rule="evenodd" clip-rule="evenodd" d="M25 0c-6.667 0-10.833 3.382-12.5 10.146 2.5-3.382 5.417-4.65 8.75-3.805 1.902.482 3.261 1.882 4.766 3.432 2.45 2.524 5.288 5.445 11.484 5.445 6.667 0 10.833-3.382 12.5-10.145-2.5 3.382-5.417 4.65-8.75 3.804-1.902-.482-3.261-1.882-4.766-3.431C34.034 2.922 31.196 0 25 0ZM12.5 15.218C5.833 15.218 1.667 18.6 0 25.364c2.5-3.382 5.417-4.65 8.75-3.805 1.902.483 3.261 1.883 4.766 3.432 2.45 2.524 5.288 5.445 11.484 5.445 6.667 0 10.833-3.381 12.5-10.145-2.5 3.382-5.417 4.65-8.75 3.805-1.902-.483-3.261-1.883-4.766-3.432-2.45-2.524-5.288-5.446-11.484-5.446Z" fill="currentColor"/></g><defs><clipPath id="a"><rect width="50" height="31" fill="currentColor"/></clipPath></defs></svg>
                            </div>

                            <h2 class="mt-6 text-xl font-semibold text-light-900 dark:text-white">Tailwind CSS</h2>

                            <p class="mt-4 text-sm leading-relaxed text-light-500 dark:text-light-400">
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" class="self-center w-6 h-6 mx-6 shrink-0 stroke-primary-500">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12h15m0 0l-6.75-6.75M19.5 12l-6.75 6.75" />
                        </svg>
                    </a>
                    <a href="https://alpinejs.dev/docs" target="_blank" class="scale-100 p-6 bg-white dark:bg-light-800/50 dark:bg-gradient-to-bl from-light-700/50 via-transparent dark:ring-1 dark:ring-inset dark:ring-white/5 rounded-lg shadow-2xl shadow-light-500/20 dark:shadow-none flex motion-safe:hover:scale-[1.01] transition-all duration-250 focus:outline focus:outline-2 focus:outline-primary-500">
                        <div>
                            <div class="flex items-center justify-center w-16 h-16 rounded-full bg-primary-50 dark:bg-primary-900/20">
                                <svg class="text-primary-400 fill-current w-7 h-7" viewBox="0 0 55 25" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="m42.753 0 12.112 12.06-12.112 12.058L30.641 12.06 42.753 0Z" fill="currentColor"/><path fill-rule="evenodd" clip-rule="evenodd" d="m12.473 0 25.11 25H13.358L.36 12.06 12.473 0Z" fill="currentColor"/></svg>
                            </div>

                            <h2 class="mt-6 text-xl font-semibold text-light-900 dark:text-white">AlpineJS</h2>
                    ...
                    <a href="https://livewire.laravel.com/docs" class="scale-100 p-6 bg-white dark:bg-light-800/50 dark:bg-gradient-to-bl from-light-700/50 via-transparent dark:ring-1 dark:ring-inset dark:ring-white/5 rounded-lg shadow-2xl shadow-light-500/20 dark:shadow-none flex motion-safe:hover:scale-[1.01] transition-all duration-250 focus:outline focus:outline-2 focus:outline-primary-500">
                        <div>
                            <div class="flex items-center justify-center w-16 h-16 rounded-full bg-primary-50 dark:bg-primary-900/20">
                            ...
                            <h2 class="mt-6 text-xl font-semibold text-light-900 dark:text-white">Livewire</h2>
            ...
            <div class="flex justify-center px-0 mt-16 sm:items-center sm:justify-between">
                <div class="text-sm text-center text-light-500 dark:text-light-400 sm:text-left">
                    <div class="flex items-center gap-4">
                        <a href="https://github.com/sponsors/taylorotwell" class="inline-flex items-center group hover:text-light-700 dark:hover:text-white focus:outline focus:outline-2 focus:rounded-sm focus:outline-primary-500">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" class="w-5 h-5 mr-1 -mt-px stroke-light-400 dark:stroke-light-600 group-hover:stroke-light-600 dark:group-hover:stroke-light-400">
                ...
                <div class="ml-4 text-sm text-center text-light-500 dark:text-light-400 sm:text-right sm:ml-0">
